...
using KrusMap = std::unordered_map<Vertex*, std::pair<Vertex*, int>>;
...
class Graph{
    private:
    GraphMap *m_vertices;
    std::vector<KrusEdges*> *m_krus_edges; // Added this at the end as a key component to Kruskal's algo
    int m_num_vertices;
    int m_num_edges;
